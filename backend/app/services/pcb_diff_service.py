...
def _pad_points(item: list, x: float, y: float, rot: float) -> list[dict]:
    """Where this footprint's pads sit in board coordinates.

    Pad `at` is local to the footprint and rotates with it, so the local point is rotated
    by the footprint angle before being translated. KiCad rotation is counter-clockwise in
    a y-down space, which is the sign convention below.

    Used to tell which tracks land on which component. That question needs real pad
    positions: the footprint origin is often not under any pad, and a bounding box claims
    every track merely passing underneath.
    """
    points: list[dict] = []
    angle = math.radians(rot)
    cos_a, sin_a = math.cos(angle), math.sin(angle)
    for pad in _get_all(item, "pad"):
        at_node = _get(pad, "at")
        if not at_node or len(at_node) < 3:
            continue
        try:
            local_x = float(at_node[1])
            local_y = float(at_node[2])
        except (ValueError, TypeError):
            continue
        layers_node = _get(pad, "layers")
        layers = [str(name) for name in (layers_node[1:] if layers_node else [])]
        points.append(
            {
                "number": pad[1] if len(pad) > 1 and isinstance(pad[1], str) else "",
                "x": x + local_x * cos_a - local_y * sin_a,
                "y": y + local_x * sin_a + local_y * cos_a,
                "layers": layers,
            }
        )
    return points
...
def _extract_footprints(tree: list) -> dict:
    ...
    for item in _get_all(tree, "footprint"):
        ...
        result[uid] = {
            ...
            "layer": layer,
            "pad_sig": pad_sig,
            # Board-space pad positions. Not compared (pad_sig already covers pad changes);
            # carried so the merge can tell which tracks attach to this component.
            "pad_points": _pad_points(item, x, y, rot),
        }
    return result
