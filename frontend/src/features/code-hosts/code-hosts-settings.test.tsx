...
vi.mock("@/features/tracker-integration/connector-settings", () => ({
    ConnectorSettings: () => <div>GitHub App form</div>,
}));
...
beforeEach(() => {
    ...
    mocks.listConnectors.mockResolvedValue([
        { ...base, id: "cn_gh", provider: "github", instanceKind: "github.com", displayName: "GitHub", host: "github.com",
            writesEnabled: true, capabilities: { issues: true, accountLinking: false } },
        { ...base, id: "cn_gl", provider: "gitlab", instanceKind: "self-hosted", displayName: "Acme GitLab",
            host: "gitlab.acme.io", capabilities: { issues: false, accountLinking: true } },
    ]);
});
...
it("walks an admin through registering a self-managed GitLab", async () => {
    mocks.createConnector.mockResolvedValue({
        ...base, id: "cn_new", provider: "gitlab", instanceKind: "self-hosted", displayName: "git.corp.io",
    ...
    fireEvent.click(screen.getByRole("radio", { name: /Self-managed/ }));

    const save = screen.getByRole("button", { name: "Add GitLab" });
    ...
    expect(screen.getByRole("link", { name: /Applications on git.corp.io/ }).getAttribute("href"))
        .toBe("https://git.corp.io/-/user_settings/applications");
    expect(screen.getByText(CALLBACK)).toBeTruthy();
    expect((save as HTMLButtonElement).disabled).toBe(true);

    fireEvent.change(screen.getByLabelText("Application ID"), { target: { value: "app-id" } });
    fireEvent.change(screen.getByLabelText("Secret"), { target: { value: "app-secret" } });
...
it("uses the GitHub App form for GitHub", async () => {
    render(<CodeHostsSettings />);
    fireEvent.click(await screen.findByRole("button", { name: /^GitHub github\.com/ }));
    expect(screen.getByText("GitHub App form")).toBeTruthy();
});
